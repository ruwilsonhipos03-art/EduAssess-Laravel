                ...
                <div class="modal-content border-0 shadow-lg">
                    <div class="modal-header bg-dark text-white border-0">
                        <div>
                            ...
                            <p class="mb-0 small text-white-50">{{ selectedExam?.Exam_Type || 'Entrance exam' }}</p>
                        </div>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body bg-light p-4">
                        <div v-if="selectedExam" class="row g-4">
                            ...
                            <div v-if="workspaceSection === 'answers'" class="col-12 col-lg-7">
                            ...
                            <div v-if="workspaceSection === 'answers'" class="col-12 col-lg-5">
                                <div class="card border-0 shadow-sm">
                                    <div class="card-header bg-white border-0 pt-4 px-4">
                                        <h6 class="fw-bold mb-1">Exam Subject</h6>
                                        <p class="text-muted small mb-0">Set the subject and question range for this
                                            exam.</p>
                                    </div>
                                    <div class="card-body px-4 pb-4">
                                        <div class="small text-muted mb-3">You can assign multiple subject ranges to one
                                            exam.
                                        </div>
                                        <div class="subject-stack">
                                            <div v-for="(row, index) in subjectForm.rows" :key="row.uid"
                                                class="subject-row-card">
                                                <div class="d-flex justify-content-between align-items-center mb-3">
                                                    <div class="fw-semibold small text-dark">Subject {{ index + 1 }}
                                                    </div>
                                                    <button type="button" class="btn btn-light-danger btn-sm"
                                                        @click="removeSubjectRow(index)"
                                                        :disabled="subjectForm.rows.length === 1">
                                                        <i class="bi bi-trash3"></i>
                                                    </button>
                                                </div>
                                                <div class="mb-3">
                                                    <label
                                                        class="form-label small fw-bold text-secondary">SUBJECT</label>
                                                    <select v-model="row.subject_id" class="form-select border-2">
                                                ...
                                                <div class="row g-3">
                                                    <div class="col-6">
                                                        <label
                                                            class="form-label small fw-bold text-secondary">FROM</label>
                                                        <input v-model.number="row.Starting_Number" type="number"
                                                            min="1" class="form-control border-2">
                                                    </div>
                                                    <div class="col-6">
                                                        <label
                                                            class="form-label small fw-bold text-secondary">TO</label>
                            ...
                            <div v-if="workspaceSection === 'sheets'" class="col-12">
                                <div class="card border-0 shadow-sm mb-4">
                                    <div class="card-header bg-white border-0 pt-4 px-4">
                                        ...
                                        <p class="text-muted small mb-0">Select a schedule, then generate pre-filled answer sheets for every student assigned to it.</p>
                                    </div>
                                    <div class="card-body px-4 pb-4">
                                        <div class="alert alert-light border mb-3">
                                            <div class="small text-muted text-uppercase fw-bold mb-1">Selected Exam</div>
                                            <div class="fw-semibold text-dark">{{ selectedExam?.Exam_Title || 'No exam selected' }}</div>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label small fw-bold text-secondary">ENTRANCE EXAM SCHEDULE</label>
                                            <select v-model="scheduleForm.exam_schedule_id" class="form-select border-2">
                                                <option value="">Select exam schedule</option>
                                                <option v-for="schedule in entranceSchedules" :key="schedule.id"
                                                    :value="String(schedule.id)">
                                                    {{ schedule.label }}
                                                </option>
                                            </select>
                                        </div>
                                        <div v-if="selectedSchedule" class="scheduled-summary-card mb-3">
                                            <div class="fw-semibold text-dark mb-1">{{ selectedSchedule.formatted_date }} | {{ selectedSchedule.time }} | {{ selectedSchedule.location }}</div>
                                            <div class="small text-muted">
                                                {{ selectedSchedule.assigned_students }} student{{ selectedSchedule.assigned_students === 1 ? '' : 's' }} assigned to this schedule will be included.
                                            </div>
                                        </div>
                                        <div v-else class="text-muted small mb-3">
                                            No entrance exam schedules are available yet.
                                        </div>
                                        <button @click="generateSheets" class="btn btn-emerald w-100 fw-bold"
                                            ...
                                            <span v-if="isGeneratingSheets"
                                                class="spinner-border spinner-border-sm me-2"></span>
                                            {{ isGeneratingSheets ? 'GENERATING...' : 'GENERATE / DOWNLOAD PRE-FILLED SHEETS' }}
                                        </button>
                                    </div>
                                </div>
                                    ...
                                    <div class="card-body px-4 pb-4">
                                        ...
                                        <div v-else class="sheet-list">
                                            <div v-for="sheet in selectedExamSheets" :key="sheet.id" class="sheet-item">
                                                <div>
                                                    <div class="fw-semibold small text-dark">{{ sheetOwnerName(sheet) }}</div>
                                                    <div class="text-muted x-small">{{ sheet.qr_payload }}</div>
...
const normalizedSubjectRows = computed(() => subjectForm.rows.map((row) => ({
    subject_id: Number(row.subject_id || 0),
    Starting_Number: Number(row.Starting_Number || 0),
    Ending_Number: Number(row.Ending_Number || 0),
})));

const subjectValidationError = computed(() => {
    const rows = normalizedSubjectRows.value;
    if (rows.length === 0) return 'Please add at least one exam subject.';
    if (rows.some((row) => !row.subject_id)) return 'Please select a subject for every row.';
    if (rows.some((row) => row.Ending_Number <= row.Starting_Number)) return 'Each "To" value must be greater than its "From" value.';
    const uniqueIds = new Set(rows.map((row) => row.subject_id));
    if (uniqueIds.size !== rows.length) return 'Please choose each subject only once per exam.';
    return '';
});

const answerKeyValidationError = computed(() => hasCompleteAnswers() ? '' : 'Please complete all 100 answer items before saving.');

const canSaveExamConfiguration = computed(() => !subjectValidationError.value && !answerKeyValidationError.value);
...
const fetchAnswerKeys = async () => {
    ...
    answerKeys.value = (Array.isArray(response.data) ? response.data : []).filter((key) =>
...
const fetchEntranceSchedules = async () => {
    const response = await axios.get('/api/entrance/schedules', {
        params: { exam_id: Number(selectedExam.value?.id || 0) || undefined },
    });

    const schedules = Array.isArray(response.data?.data) ? response.data.data : [];
    entranceSchedules.value = schedules;

    const hasCurrent = schedules.some((schedule) => Number(schedule.id) === Number(scheduleForm.exam_schedule_id));
    if (!hasCurrent) {
        const firstSchedule = schedules.find((schedule) => Number(schedule.assigned_students || 0) > 0) || schedules[0] || null;
        scheduleForm.exam_schedule_id = firstSchedule ? String(firstSchedule.id) : '';
    }
};
...
const fetchSubjects = async () => {
    try {
        ...
        try {
            const fallback = await axios.get('/api/admin/subjects');
            subjects.value = Array.isArray(fallback.data?.data) ? fallback.data.data : [];
        } catch (_) {
            subjects.value = [];
        }
    }
...
const createSubjectRow = (row = {}, fallbackStart = 1) => {
    const startingNumber = Number(row?.Starting_Number || fallbackStart || 1);
    const endingNumber = Number(row?.Ending_Number || Math.max(startingNumber, 100));
...
const syncSubjectForm = () => {
    const rows = Array.isArray(selectedExam.value?.exam_subjects) ? selectedExam.value.exam_subjects : [];
    if (rows.length > 0) {
        let nextStart = 1;
        subjectForm.rows = rows.map((row) => {
            const mapped = createSubjectRow(row, nextStart);
            nextStart = Number(mapped.Ending_Number || nextStart) + 1;
            return mapped;
        });
        return;
    }

    subjectForm.rows = [createSubjectRow({}, 1)];
};

const addSubjectRow = () => {
    if (subjectForm.rows.length === 0) {
        subjectForm.rows.push(createSubjectRow({}, 1));
        return;
    }

    const lastRow = subjectForm.rows[subjectForm.rows.length - 1];
    const nextStart = Number(lastRow?.Ending_Number || 0) + 1;
    subjectForm.rows.push(createSubjectRow({}, nextStart > 0 ? nextStart : 1));
};

const removeSubjectRow = (index) => {
    if (subjectForm.rows.length === 1) return;
    subjectForm.rows.splice(index, 1);
};

const sheetOwnerName = (sheet) => {
    const first = String(sheet?.user?.first_name || '').trim();
    const last = String(sheet?.user?.last_name || '').trim();
    const ext = String(sheet?.user?.extension_name || '').trim();
    const fullName = [last ? `${last},` : '', first, ext].filter(Boolean).join(' ').trim();
    return fullName || 'Unassigned sheet';
};

const refreshWorkspaceData = async () => {
    isLoading.value = true;
    try {
        await Promise.all([fetchExams(), fetchAnswerKeys(), fetchSheets(), fetchSubjects()]);
...
const saveExamConfiguration = async () => {
    ...
    if (subjectValidationError.value) {
        window.Swal.fire({ icon: 'warning', title: 'Invalid Exam Subject', text: subjectValidationError.value });
        return;
    }
    ...
    try {
        await axios.put(`/api/exams/${selectedExam.value.id}`, {
            Exam_Title: selectedExam.value.Exam_Title,
            Exam_Type: selectedExam.value.Exam_Type,
            exam_subjects: normalizedSubjectRows.value,
        });
...
const generateSheets = async () => {
    ...
    if (!scheduleForm.exam_schedule_id) {
        window.Swal.fire({ icon: 'warning', title: 'No Exam Schedule', text: 'Please select an entrance exam schedule first.' });
        return;
    }
