                ...
                <div class="action-card card border-0 shadow-sm p-4 rounded-4 bg-white">
                    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                        ...
                        <button class="btn btn-scan d-flex align-items-center gap-3 px-5 py-3 rounded-3 shadow"
                            :disabled="isScanning" @click="openCameraModal">
...
const initializeCamera = async (sessionId = cameraSessionId.value) => {
    ...
    try {
        ...
        if (!isCameraSessionActive(sessionId)) return;
...
const captureFrameAsFile = async () => {
    ...
    formData.append('image', new File([blob], `capture-${Date.now()}.jpg`, { type: 'image/jpeg' }));
    return { file: formData.get('image'), reason: '' };
};
